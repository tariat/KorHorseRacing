...
"""
출마표 수집 스크립트.
step01_collect_race_result.py 흐름을 따라
 - URL에서 출마표 테이블을 읽고
...
def collect_entry(location: int, bas_dt: str, rc_no: int, try_cnt: int = 1) -> pd.DataFrame:
    ...
    url = (
        "http://race.kra.co.kr/chulmainfo/chulmaDetailInfoChulmapyo.do?"
    ...
    # 출마표 테이블: 마번/마명 등이 포함된 첫 번째 테이블을 사용
    for table in tables:
        df_candidate = table.copy()
    ...
    entry_df = entry_df.rename(columns={"번호▲": "번호"})
...
def insert_table(entry_df: pd.DataFrame, table_name: str = "hr_entry") -> int:
    """
    수집된 DataFrame을 MySQL에 적재한다.
    :param entry_df: 수집 데이터
    :param db_conf: DB 연결 정보. 미지정 시 step01 기본값 사용
...
if __name__ == "__main__":
    ...
    # DB 적재
    insert_table(df_entry, "hr_entry")
