...
def collect_weight(location: int, bas_dt: str, rc_no: int, try_cnt: int = 1) -> pd.DataFrame:
    if try_cnt > 3:
    ...
    url = (
        "https://race.kra.co.kr/chulmainfo/chulmaDetailInfoWeight.do?"
    ...
    try:
        ...
        return collect_weight(location, bas_dt, rc_no, try_cnt + 1)
...
def insert_table(df: pd.DataFrame, table_name: str = "hr_weight") -> int:
...
if __name__ == "__main__":
    ...
    sample_dt = "20251122"
    df_weight = collect_weight(1, sample_dt, 1, 1)
    print(df_weight.head())
    insert_table(df_weight)
