...
def collect_match_record(location: int, bas_dt: str, rc_no: int, try_cnt: int = 1) -> pd.DataFrame:
    if try_cnt > 3:
    ...
    url = (
        "https://race.kra.co.kr/chulmainfo/chulmaDetailInfoMatchRecordList.do?"
    ...
    try:
        ...
        return collect_match_record(location, bas_dt, rc_no, try_cnt + 1)
...
def insert_table(df: pd.DataFrame, table_name: str = "hr_match_record") -> int:
...
if __name__ == "__main__":
    ...
    sample_dt = "20251122"
    df_match = collect_match_record(1, sample_dt, 1, 1)
    print(df_match.head())
    insert_table(df_match)
