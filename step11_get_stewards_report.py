...
def collect_stewards_report(location: int, bas_dt: str, rc_no: int, try_cnt: int = 1) -> pd.DataFrame:
    if try_cnt > 3:
    ...
    url = (
        "https://race.kra.co.kr/chulmainfo/chulmaDetailInfoStewardsReport.do?"
    ...
    try:
        ...
        return collect_stewards_report(location, bas_dt, rc_no, try_cnt + 1)
...
def insert_table(df: pd.DataFrame, table_name: str = "hr_stewards_report") -> int:
...
if __name__ == "__main__":
    ...
    sample_dt = "20251122"
    df_report = collect_stewards_report(1, sample_dt, 1, 1)
    print(df_report.head())
    insert_table(df_report)
